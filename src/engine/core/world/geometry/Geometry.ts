...
export class Geometry extends TObject{
     ...
     * @return {Bounds} 包围盒数据
     */
    getBounds(matrix = null, bounds ?: any) {
        if (!bounds) {
            bounds = {
                xMin: Infinity,
                xMax: -Infinity,
    ...
    private _localSphereBounds : any;
    ...
    convertToNoIndices(verticesItemLen = 3) {
        ...
        for (let i = 0; i < indicesLen; i++) {
            ...
            Utils.copyArrayData(skinIndices, this.skinIndices, i * 4, idx * 4, 4);
            Utils.copyArrayData(skinWeights, this.skinWeights, i * 4, idx * 4, 4);
            if (this.colors) {
